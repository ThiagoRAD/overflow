...
const TaskItem = ({ task }) => {
  ...
  return (
    <div 
      className="p-4 border-2 rounded-2xl flex justify-between items-center cursor-pointer transition-all duration-200 hover:bg-white/5 hover:shadow-lg text-lg box-sh"
    ...
    >
      ...
      <div className="flex items-center gap-2 text-sm">
        <div className="w-12 h-12 relative">
          ...
          <div className='absolute flex gap-1 items-center justify-end right-[-3px] bottom-0 text-[10px]'>{task?.timesCompleted}</div>
          <Pomodoro task={task} />
        </div>
      </div>
    </div>
